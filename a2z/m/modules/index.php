<html>
<head>
<title>Playful Invention and Exploration - Motion Modules</title>
<meta http-equiv="Content-Type" content="text/html; charset=iso-8859-1">
<link rel="stylesheet" href="../../../css/main.css">
<script language="JavaScript">
<!--
function MM_swapImgRestore() { //v3.0
  var i,x,a=document.MM_sr; for(i=0;a&&i<a.length&&(x=a[i])&&x.oSrc;i++) x.src=x.oSrc;
}

function MM_preloadImages() { //v3.0
  var d=document; if(d.images){ if(!d.MM_p) d.MM_p=new Array();
    var i,j=d.MM_p.length,a=MM_preloadImages.arguments; for(i=0; i<a.length; i++)
    if (a[i].indexOf("#")!=0){ d.MM_p[j]=new Image; d.MM_p[j++].src=a[i];}}
}

function MM_findObj(n, d) { //v4.01
  var p,i,x;  if(!d) d=document; if((p=n.indexOf("?"))>0&&parent.frames.length) {
    d=parent.frames[n.substring(p+1)].document; n=n.substring(0,p);}
  if(!(x=d[n])&&d.all) x=d.all[n]; for (i=0;!x&&i<d.forms.length;i++) x=d.forms[i][n];
  for(i=0;!x&&d.layers&&i<d.layers.length;i++) x=MM_findObj(n,d.layers[i].document);
  if(!x && d.getElementById) x=d.getElementById(n); return x;
}

function MM_swapImage() { //v3.0
  var i,j=0,x,a=MM_swapImage.arguments; document.MM_sr=new Array; for(i=0;i<(a.length-2);i+=3)
   if ((x=MM_findObj(a[i]))!=null){document.MM_sr[j++]=x; if(!x.oSrc) x.oSrc=x.src; x.src=a[i+2];}
}
//-->
</script>


</head>

<body bgcolor="#FFFFFF" text="#000000" onload="MM_preloadImages('../../../images/navi/home-over.gif','../../../images/navi/a2zOVER.gif','../../../images/navi/cricketOVER.gif','../../../images/navi/scrapbookOVER.gif','../../../images/navi/resourcesOVER.gif','../../../images/navi/sign_over.gif','../../../images/navi/balloon_over.gif')">

<table width="700" border="0" cellspacing="0" cellpadding="5">
<tr>

<td rowspan="2" width="70" align="center" valign="top">

<a href="../../../" onmouseout="MM_swapImgRestore()" onmouseover="MM_swapImage('home','','../../../images/navi/home-over.gif',1)"><img name="home" border="0" src="../../../images/navi/home-up.gif" width="50" height="50" align="top"></a><br>
<a href="../../../" onmouseout="MM_swapImgRestore()" onmouseover="MM_swapImage('home','','../../../images/navi/home-over.gif',1)">Home</a><br>
<img src="../../../images/navi/small-line.jpg" width="54" height="8" align="middle"><br>

<a href="../../../a2z/" onmouseout="MM_swapImgRestore()" onmouseover="MM_swapImage('a2z','','../../../images/navi/a2zOVER.gif',1)"><img src="../../../images/navi/a2zICON.gif" name="a2z" width="44" height="23" border="0" id="a2z"></a><br>
<a href="../../../a2z/" onmouseout="MM_swapImgRestore()" onmouseover="MM_swapImage('a2z','','../../../images/navi/a2zOVER.gif',1)">Index</a><br>
<img src="../../../images/navi/small-line.jpg" width="54" height="8"><br>

<a href="../../../crickets/" onmouseout="MM_swapImgRestore()" onmouseover="MM_swapImage('cricket','','../../../images/navi/cricketOVER.gif',1)"><img src="../../../images/navi/cricketICON.gif" name="cricket" width="50" height="47" border="0" id="cricket"></a><br>
<a href="../../../crickets/" onmouseout="MM_swapImgRestore()" onmouseover="MM_swapImage('cricket','','../../../images/navi/cricketOVER.gif',1)">Crickets</a><br>
<img src="../../../images/navi/small-line.jpg" width="54" height="8"><br>

<a href="../../../events/" onmouseout="MM_swapImgRestore()" onmouseover="MM_swapImage('scrapbook','','../../../images/navi/scrapbookOVER.gif',1)"><img name="scrapbook" border="0" src="../../../images/navi/scrapbookICON.gif" width="59" height="40"></a><br>
<a href="../../../events/" onmouseout="MM_swapImgRestore()" onmouseover="MM_swapImage('scrapbook','','../../../images/navi/scrapbookOVER.gif',1)">Events</a><br>
<img src="../../../images/navi/small-line.jpg" width="54" height="8"><br>

<a href="../../../ideas/" onmouseout="MM_swapImgRestore()" onmouseover="MM_swapImage('resources','','../../../images/navi/resourcesOVER.gif',1)"><img name="resources" border="0" src="../../../images/navi/resourcesICON.gif" height="40"></a><br>
<a href="../../../ideas/" onmouseout="MM_swapImgRestore()" onmouseover="MM_swapImage('resources','','../../../images/navi/resourcesOVER.gif',1)">Ideas</a><br>
<img src="../../../images/navi/small-line.jpg" width="54" height="8"><br>

<a href="../../../locations/" onmouseout="MM_swapImgRestore()" onmouseover="MM_swapImage('sign11','','../../../images/navi/sign_over.gif',1)"><img src="../../../images/navi/sign_up.gif" name="sign11" width="60" height="58" border="0" id="sign1"></a><br>
<a href="../../../locations/" onmouseout="MM_swapImgRestore()" onmouseover="MM_swapImage('sign11','','../../../images/navi/sign_over.gif',1)">Locations</a><br>
<img src="../../../images/navi/small-line.jpg" width="54" height="8"><br>

<a href="../../../philosophy/" onmouseout="MM_swapImgRestore()" onmouseover="MM_swapImage('balloon11','','../../../images/navi/balloon_over.gif',1)"><img src="../../../images/navi/balloon_up.gif" name="balloon11" width="60" height="38" border="0" id="balloon1"></a><br>
<a href="../../../philosophy/" onmouseout="MM_swapImgRestore()" onmouseover="MM_swapImage('balloon11','','../../../images/navi/balloon_over.gif',1)">Philosophy</a><br>
<img src="../../../images/navi/small-line.jpg" width="54" height="8">

<br><br><br>

<table class="bigIdeaBox">
<tr><td>
<span class="style4">For those times when you don't want to spend a lot of time figuring out how to build a mechanism, and would rather focus on programming or other aspects of the design.</span>
</td></tr>
</table>

</td>
<td align="center" valign="top">


<br>
<img src="../../../a2z/images/alphabets/AlphaupColor.jpg" width="550" height="29" border="0" usemap="#map"> 
<map name="map">
<area shape="rect" coords="0,0,21,21" href="../../../a2z/#a">
<area shape="rect" coords="26,0,42,21" href="../../../a2z/#b">
<area shape="rect" coords="46,0,64,21" href="../../../a2z/#c">
<area shape="rect" coords="67,0,86,21" href="../../../a2z/#d">
<area shape="rect" coords="89,0,103,21" href="../../../a2z/#e">
<area shape="rect" coords="107,0,121,21" href="../../../a2z/#f">
<area shape="rect" coords="126,0,142,21" href="../../../a2z/#g">
<area shape="rect" coords="147,0,164,21" href="../../../a2z/#h">
<area shape="rect" coords="169,0,181,21" href="../../../a2z/#i">
<area shape="rect" coords="184,0,196,21" href="../../../a2z/#j">
<area shape="rect" coords="197,0,214,21" href="../../../a2z/#k">
<area shape="rect" coords="220,0,235,21" href="../../../a2z/#l">
<area shape="rect" coords="239,0,258,21" href="../../../a2z/#m">
<area shape="rect" coords="262,0,278,21" href="../../../a2z/#n">
<area shape="rect" coords="283,0,302,21" href="../../../a2z/#o">
<area shape="rect" coords="306,0,323,21" href="../../../a2z/#p">
<area shape="rect" coords="326,0,345,26" href="../../../a2z/#q">
<area shape="rect" coords="350,0,367,21" href="../../../a2z/#r">
<area shape="rect" coords="370,0,388,21" href="../../../a2z/#s">
<area shape="rect" coords="391,0,408,21" href="../../../a2z/#t">
<area shape="rect" coords="410,0,429,21" href="../../../a2z/#u">
<area shape="rect" coords="433,0,452,21" href="../../../a2z/#v">
<area shape="rect" coords="455,0,486,21" href="../../../a2z/#w">
<area shape="rect" coords="488,0,504,21" href="../../../a2z/#x">
<area shape="rect" coords="509,0,529,21" href="../../../a2z/#y">
<area shape="rect" coords="530,0,548,21" href="../../../a2z/#z">
</map>

<br>
<img src="../../../a2z/images/line.gif" width="601" height="12">

<h1 class="style3">Motion Modules</h1>

<script language="JavaScript" type="text/JavaScript">
<!--
function MM_swapImgRestore() { //v3.0
  var i,x,a=document.MM_sr; for(i=0;a&&i<a.length&&(x=a[i])&&x.oSrc;i++) x.src=x.oSrc;
}

function MM_preloadImages() { //v3.0
  var d=document; if(d.images){ if(!d.MM_p) d.MM_p=new Array();
    var i,j=d.MM_p.length,a=MM_preloadImages.arguments; for(i=0; i<a.length; i++)
    if (a[i].indexOf("#")!=0){ d.MM_p[j]=new Image; d.MM_p[j++].src=a[i];}}
}

function MM_findObj(n, d) { //v4.01
  var p,i,x;  if(!d) d=document; if((p=n.indexOf("?"))>0&&parent.frames.length) {
    d=parent.frames[n.substring(p+1)].document; n=n.substring(0,p);}
  if(!(x=d[n])&&d.all) x=d.all[n]; for (i=0;!x&&i<d.forms.length;i++) x=d.forms[i][n];
  for(i=0;!x&&d.layers&&i<d.layers.length;i++) x=MM_findObj(n,d.layers[i].document);
  if(!x && d.getElementById) x=d.getElementById(n); return x;
}

function MM_swapImage() { //v3.0
  var i,j=0,x,a=MM_swapImage.arguments; document.MM_sr=new Array; for(i=0;i<(a.length-2);i+=3)
   if ((x=MM_findObj(a[i]))!=null){document.MM_sr[j++]=x; if(!x.oSrc) x.oSrc=x.src; x.src=a[i+2];}
}
//-->
</script>
<body onLoad="MM_preloadImages('images/animations/bigbob.gif','images/animations/flowerturn.gif','images/animations/piston.gif','images/animations/reacher.gif')">

<table width="650" border="0" cellspacing="0" cellpadding="0">
<tr>

<td><img src="images/reacher_animate.gif" width="280" height="250"></td>

<td>
<table width="360" border="0" cellpadding="0" cellspacing="0">
<tr>
<td><img src="../../../a2z/images/box_nw.gif" width="30" height="30" alt=""></td>
<td><img src="../../../a2z/images/box_n.gif" width="300" height="30" alt=""></td>
<td><img src="../../../a2z/images/box_ne.gif" width="30" height="30" alt=""></td>
</tr><tr>
<td background="../../../a2z/images/box_ew.gif"></td>
<td><p class="style1">Looking for a way to make something move? Choose a motion, then build it using a small set of LEGO parts. - <em>Natalie</em></p></td>
<td background="../../../a2z/images/box_ew.gif"></td>
</tr><tr>
<td><img src="../../../a2z/images/box_sw.gif" width="30" height="30" alt=""></td>
<td><img src="../../../a2z/images/box_s.gif" width="300" height="30" alt=""></td>
<td><img src="../../../a2z/images/box_se.gif" width="30" height="30" alt=""></td>
</tr>
</table>
</td>

</tr>
</table>

<table width="650" border="0" cellspacing="0" cellpadding="0">
<tr>

<td align="center">



<!-- box top code -->
<table border="0" cellpadding="0" cellspacing="0">
<tr>
<td><img src="../../../a2z/images/box_nw.gif" width="30" height="30" alt=""></td>
<td><img src="../../../a2z/images/box_n.gif" width="550" height="30" alt=""></td>
<td><img src="../../../a2z/images/box_ne.gif" width="30" height="30" alt=""></td>
</tr><tr>
<td background="../../../a2z/images/box_ew.gif"></td>
<td>
<!-- end box top code -->


<table width="550" border="0" align="center" cellpadding="5" cellspacing="5" class="howToText">
  <tr>
    <td width="25%" align="center"><div align="center"><a href="clapper/index.php" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('clapping-rollover','','images/animations/clacker_fin.gif',0)"><img src="images/clacker_fin.jpg" alt="Clapping Motion Module animation" name="clapping-rollover" width="120" height="90" border="0"></a><br>
            <a href="clapper/index.php">Clapper</a></div></td>
    <td width="25%" align="center"><div align="center"><a href="piston1/index.php" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('bigbobrollover','','images/animations/bigbob.gif',1)"><img src="images/bigbob.jpg" alt="piston 1  motion module animation" name="bigbobrollover" width="120" height="90" border="0"></a><br>
            <a href="piston1/index.php">Piston 1</a></div></td>
    <td width="25%" align="center">
      <p align="center"><a href="turn/index.html" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('turn-rollover','','images/animations/flowerturn.gif',1)"> </a><a href="piston2/index.php" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('piston-rollover','','images/animations/piston.gif',1)"><img src="images/piston.jpg" alt="Piston 2 Motion Module animation" name="piston-rollover" width="120" height="90" border="0"></a><br>
          <a href="piston2/index.php" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('rack2-rollover','','images/animations/rack2.gif',1)">Piston 2</a></p></td>
    <td width="25%" align="center"><p align="center"><a href="popper/index.php" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('popper-rollover','','images/animations/popper.gif',0)"><img src="images/popper.jpg" alt="Popper Motion Module" name="popper-rollover" width="120" height="90" border="0"></a><br>
            <a href="popper/index.php">Popper</a></p></td>
  </tr>
  <tr>
    <td align="center"><a href="rack1/index.php" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('rack1-rollover','','images/animations/rack1.gif',0)"><img src="images/rack1.jpg" alt="Rack Animation version 1" name="rack1-rollover" width="120" height="90" border="0"><br>
      </a><a href="rack1/index.php" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('rack2-rollover','','images/animations/rack2.gif',1)">Rack 1</a></td>
    <td align="center"><a href="reacher/index.php" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('reacher-rollover','','images/animations/reacher.gif',1)"><img src="images/reacher.jpg" alt="Reacher Animation" name="reacher-rollover" width="120" height="90" border="0"><br>
      </a><a href="reacher/index.php">Reacher</a><a href="rack2/index.html" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('rack2-rollover','','images/animations/rack2.gif',1)"><br>
    </a></td>
    <td align="center"><a href="reel/index.php" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('reel-rollover','','images/animations/reel4.gif',0)"><img src="images/reel4.jpg" alt="Reel Motion Module Animation" name="reel-rollover" width="120" height="90" border="0"><br>
      </a><a href="reel/index.php">Reel</a><br>    </td>
    <td align="center"><a href="lifter/index.php" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('lifter-rollover','','images/animations/lifter-ani120.gif',0)"><img src="images/lifter-ani120.jpg" alt="Lifter Motion Module Animation" name="lifter-rollover" width="120" height="90" border="0"><br>
    </a><a href="lifter/index.php">Lifter</a></td>
  <tr>
    <td align="center"><a href="stirrer/index.php" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('stirrer-rollover','','images/animations/stirer.gif',0)"><img src="images/stirrer.jpg" alt="Stirrer Motion Module Animation" name="stirrer-rollover" width="120" height="90" border="0"></a><br>
        <a href="stirrer/index.php">Stirrer</a></td>
    <td align="center"><a href="spin1/index.php" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('turn-rollover','','images/animations/flowerturn.gif',1)"><img src="images/flowerturn.jpg" alt="Spinning Motion Module" name="turn-rollover" width="120" height="90" border="0"><br>
      Spin 1</a></td>
    <td align="center"><a href="spin3/index.php" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('spin3','','images/animations/turner.gif',0)"><img src="images/carousel.jpg" alt="spinning motion module" name="spin3" width="120" height="90" border="0"></a><a href="spin3/index.php">Spin 2 </a> </td>
    <td align="center"><a href="spin-module/index.php" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('mini-turn-rollover','','images/animations/miniturn.gif',1)"><img src="images/turntable.jpg" width="120" height="90" border="0"><br>
      </a><a href="spin-module/index.php" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('turn-rollover','','images/animations/flowerturn.gif',1)">Turntable</a> </td>
  </tr>
</table>
<!-- box bottom code -->
</td>
<td background="../../../a2z/images/box_ew.gif" ></td>
</tr><tr>
<td><img src="../../../a2z/images/box_sw.gif" width="30" height="30" alt=""></td>
<td><img src="../../../a2z/images/box_s.gif" width="550" height="30" alt=""></td>
<td><img src="../../../a2z/images/box_se.gif" width="30" height="30" alt=""></td>
</tr>
</table>
<!-- end box bottom code -->


<!-- this is the link box -->
<table border="0" cellpadding="0" cellspacing="0">
<tr>
<td><img src="../../../a2z/images/box_nw.gif" width="30" height="30" alt=""></td>
<td><img src="../../../a2z/images/box_n.gif" width="300" height="30" alt=""></td>
<td><img src="../../../a2z/images/box_ne.gif" width="30" height="30" alt=""></td>
</tr><tr>
<td background="../../../a2z/images/box_ew.gif"></td>
<td>

<table width="300" border="0">
<tr>
<td height="69" align="center"><img src="../../../a2z/images/what_else.gif" alt="What else" width="179" height="60"></td>
</tr>
<tr>
  <td align="center">link one</td>
</tr>
<tr>
<td align="center"><img src="../../../a2z/images/shortline.gif" width="249" height="12"></td>
</tr>
<tr>
  <td align="center"><a href="http://www.ldraw.org/">Try drawing your own modules in 3D </a></td>
</tr>
<tr>
<td align="center"><img src="../../../a2z/images/shortline.gif" width="249" height="12"></td>
</tr>
<tr>
  <td align="center">link three</td>
</tr>
</table>

</td>
<td background="../../../a2z/images/box_ew.gif"></td>
</tr><tr>
<td><img src="../../../a2z/images/box_sw.gif" width="30" height="30" alt=""></td>
<td><img src="../../../a2z/images/box_s.gif" width="300" height="30" alt=""></td>
<td><img src="../../../a2z/images/box_se.gif" width="30" height="30" alt=""></td>
</tr>
</table>

<br>&nbsp;

</td>

</tr>
</table>


</td>

</tr><tr>

<td align="center" valign="bottom">

<font size="-2" face="Verdana, Arial, Helvetica, sans-serif">
<a href="../../../">Home</a> | <a href="../../../a2z/">A to Z</a> | <a href="../../../crickets/">Crickets</a> | <a href="../../../events/">Events</a> | <a href="../../../ideas/">Ideas</a> | <a href="../../../locations/">Locations</a> | <a href="../../../philosophy/">Philosophy</a>
<br><br>
PIE is the Playful Invention and Exploration Network<br>
Send additions or corrections to <a href="mailto:piewebteam&#64;smm.org">piewebteam</a>
</font>

</td>
</tr>
</table>

</body>
</html>